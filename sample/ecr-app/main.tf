...
data "terraform_remote_state" "network" {
  ...
  config = {
    ...
    key    = "sample/resources/network/your-vpc/terraform.tfstate"
...
data "terraform_remote_state" "elb" {
  ...
  config = {
    bucket = "nextpay-terraform-state"
    key    = "sample/resources/lb/your-lb/terraform.tfstate"
    region = var.aws_region
  }
}
...
data "terraform_remote_state" "iam" {
  ...
  config = {
    bucket = "nextpay-terraform-state"
    key    = "sample/resources/iam/your-ecs-roles/terraform.tfstate"
    region = var.aws_region
  }
}
  ...
  ingress_rules = [
    {
      ...
      description     = "Allow traffic on port 3827 from anywhere"
    }
  ]
...
module "route53_record" {
  ...
  count  = var.generate_route53_record ? 1 : 0
...
module "ecs_service" {
  ...
  deployment_configuration = { #Rolling Update 방식
    maximum_percent         = 200 #배포 중 최대 200%까지 태스크 실행 가능 (기존 + 새 버전)
    minimum_healthy_percent = 100 #최소 100% 태스크 유지 (무중단 배포)
