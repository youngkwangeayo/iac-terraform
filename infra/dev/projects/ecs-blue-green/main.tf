...
data "terraform_remote_state" "iam_lb_bg" {
  backend = "s3"
  config = {
    bucket = "nextpay-terraform-state"
...
# ============================================================================
locals {
  blue_green_config = var.deployment_strategy == "BLUE_GREEN" ? {
    alternate_target_group_arn = module.target_group_blue_green[0].target_group_arn
    production_listener_rule   = aws_lb_listener_rule.https.arn
    role_arn                   = data.terraform_remote_state.iam_lb_bg.outputs.arn
  } : null
}
...
module "target_group_blue_green" {
  source = "../../../modules/load-balancer/target-group"
  count  = var.deployment_strategy == "BLUE_GREEN" ? 1 : 0

  name = "${module.common.common_name}-bluegreen"
...
resource "aws_lb_listener_rule" "https" {
  ...
  action {
    ...
    forward {
      target_group {
        arn    = module.target_group.target_group_arn
        weight = 100
      }

      dynamic "target_group" {
        for_each = var.deployment_strategy == "BLUE_GREEN" ? module.target_group_blue_green : []
        content {
...
module "ecs_service" {
  ...
  blue_green_deployment = local.blue_green_config
